...
const packageTemplate = `// This file was generated by SKF/go-service-doc at
// {{ .Timestamp }}
package docs
...
func searchHandler(searchIndex bleve.Index) func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, req *http.Request) {
		...
		for idx, hit := range searchResult.Hits {
			...
			result[idx].HTML = template.HTML(hit.Fields["HTML"].(string))
...
func createSearchPage(queryString string, searchResult []document) []byte {
	buf := bytes.Buffer{}
	tmpl, err := template.New("searchPage").Funcs(template.FuncMap{"join": strings.Join}).Parse(searchPage)
	if err != nil {
		panic("Unable to parse tempalate: " + err.Error())
	}

	err = tmpl.ExecuteTemplate(&buf, "searchPage", map[string]interface{}{
		"QueryString":   queryString,
		"SearchResults": searchResult,
	})
	if err != nil {
		panic("Unable to execute tempalate: " + err.Error())
	}
